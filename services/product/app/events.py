"""RabbitMQ event publisher for Product Service."""
...
async def publish_product_event(event_type: str, product_data: dict[str, Any]) -> None:
    """
    Publish product-related events.

    Args:
        event_type: Type of event (created, updated, deleted)
        product_data: Product data to include in the event
    """
    ...
    logger.info(f"Published {routing_key} event for product {product_data.get('id')}")
